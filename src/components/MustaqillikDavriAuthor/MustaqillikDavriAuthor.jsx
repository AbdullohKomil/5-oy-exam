...
export const MustaqillikDavriAuthor = () => {
  const [data, setData] = useState([]);
  useEffect(() => {
    const getData = async () => {
      const data = await api.mustaqillikDavri().catch((err) => err);
      setData(data.data);
    };
    getData();
  }, [setData]);
    ...
    <div>
      <ul className='list-none flex gap-9 mt-9'>
        {data.length
          ? data.map((el) => {
              return (
                <li key={el.id} className=' rounded-3xl  w-72 pb-16 bg-zinc-100	 border border-gray-200 shadow  dark:bg-neutral-900 dark:border-gray-700'>
